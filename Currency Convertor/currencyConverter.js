...
const apiKey = `06c2160d0882dbe1a31399ee`;
...
function loadFlag(element){
    for(let code in country_list){
        ...
        if(code == element.value){
            ...
            let url = `https://countryflagsapi.com/png/${country_list[code]}`;
            imgTag.src = url;
        }
    }
}
...
function getExhangeRate(){
    ...
    fetch(url).then(response => response.json()).then(result => {
        ...
        exchangeRateTxt.innerText = `${amountVal} ${fromCurrency.value} = ${totalExhangeRate} ${toCurrency.value}`;
    })
